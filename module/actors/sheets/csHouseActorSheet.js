...
export class CSHouseActorSheet extends CSActorSheet {
    ...
    getData(options) {
        ...
        let house = system.actor.getCSData();

        house.historicalEvents = this._checkNull(system.itemsByType['event']);
    ...
    prepareRolesData(house, data) {
        house.head = system.actor.getCharactersFromRole(system.actor.roleMap.HEAD);
        house.steward = system.actor.getCharactersFromRole(
            system.actor.roleMap.STEWARD
        );
        house.heirs = system.actor.getCharactersFromRole(system.actor.roleMap.HEIR);
        house.family = system.actor.getCharactersFromRole(
            system.actor.roleMap.FAMILY
        );
        house.retainers = system.actor.getCharactersFromRole(
            system.actor.roleMap.RETAINER
        );
        house.servants = system.actor.getCharactersFromRole(
            system.actor.roleMap.SERVANT
        );
    }
    ...
    prepareFortuneData(house, data) {
        ...
        house.fortune = {
            lawMod: system.actor.getLawModifier(),
            populationMod: system.actor.getPopulationModifier(),
            holdingsDice: system.actor.getHoldingsDice(),
            holdingsFlat: system.actor.getHoldingsModifier(),
            holdingsMod: '0',
        };
    ...
    prepareHoldingData(house, data) {
        ...
        let holdings = this._checkNull(system.itemsByType['holding']);
    ...
    async _openResourceEditor(ev) {
        ...
        const html = await renderTemplate(template, {
            startingValue: system.actor.getCSData()[resourceId].startingValue,
            description: system.actor.getCSData()[resourceId].description,
    ...
    _processResourceEdition(formData) {
        this.actor.changeResource(
            formsystem.resourceId.value,
            formsystem.startingValue.value,
            formsystem.description.value
        );
        return true;
    }
    ...
    async _onDropActor(event, data) {
        ...
        fromUuid(system.uuid).then((value) => {
            let actor = value;
                ...
                if (doc.type === 'event') {
                    await this.showAddingEventDialog(doc).then((result) => {
                        if (!result.cancelled) {
                            ...
                            if (generatesystem.canGenerate) {
                                eventsCanGenerateModifiers.push({
                                    doc: doc.name,
                                    choices: generatesystem.choices,
                                });
                            }
    ...
    _processAddingEvent(formData) {
        if (!formsystem.system.generateModifiers.checked)
            return { canGenerate: false };

        let choices = [];
        for (let i = 1; i <= formsystem.event.system.numberOfChoices; i++) {
            choices.push(formsystem.data[`resource_${i}`].value);
        }
    ...
    _processCharacterRole(formData) {
        this.actor.addCharacterToHouse(
            formsystem.characterId.value,
            formsystem.characterRole.value,
            formsystem.description.value
        );
        return true;
    }
